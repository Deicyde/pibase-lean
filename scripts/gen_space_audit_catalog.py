...
DEFAULT_CATALOG_OUTPUT = ROOT / "PiBaseLean" / "Audit" / "Spaces" / "Catalog.lean"
DEFAULT_GENERATED_OUTPUT = (
    ROOT / "PiBaseLean" / "Audit" / "Spaces" / "GeneratedCatalog.lean"
)
SCHEMA_VERSION = 1

CATALOG_SCHEMA = """module

-- This file is generated by scripts/gen_space_audit_catalog.py.
...
def generate(pibase_path: Path, independence_path: Path) -> tuple[str, str]:
    data = load_catalog(pibase_path, independence_path)
    return render_catalog_schema(), render_generated_catalog(data)
...
def run(argv: Sequence[str] | None = None) -> int:
    ...
    catalog_source, generated_source = generate(args.pibase, args.independence)
    expected = (
        (args.catalog_output, catalog_source),
        (args.generated_output, generated_source),
    )
    if args.check:
