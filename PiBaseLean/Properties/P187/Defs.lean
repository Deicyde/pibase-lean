...
def P187 : Property where
  ...
  well_defined {X Y : Type u} [TopologicalSpace X] [TopologicalSpace Y] (φ : X ≃ₜ Y) h := by
    ...
    unfold wGame
    refine ofAllowed_isPayoff_iff (wMoveComap φ) ?_ ?_ b
    · exact wAllowed_comap_iff φ y
    · intro c
      change Tendsto (fun n => (c (2 * n + 1)).1) atTop (𝓝 y) ↔
        Tendsto (fun n => (wMoveComap φ (c (2 * n + 1))).1) atTop (𝓝 (φ.symm y))
      rw [φ.symm.isInducing.tendsto_nhds_iff (f := fun n => (c (2 * n + 1)).1)]
      simp [Function.comp_def, wMoveComap]
