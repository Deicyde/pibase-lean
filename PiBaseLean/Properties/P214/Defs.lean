...
private theorem nonempty_inter_iff (φ : X ≃ₜ Y) (f : ℕ → Y) (g : ℕ → X) :
    (range f ∩ range (φ ∘ g)).Nonempty ↔ (range (φ.symm ∘ f) ∩ range g).Nonempty := by
  rw [symm_image_range_inter φ f g, Set.image_nonempty]

private theorem infinite_setOf_nonempty_inter (φ : X ≃ₜ Y) (S : ℕ → ℕ → Y) (T : ℕ → X)
...
def P214 : Property where
  ...
  well_defined {X Y} _ _ φ h := by
    ...
    exact ⟨φ ∘ T, φ.injective.comp hT_inj, tendsto_comp_of_symm φ hT_tend,
      range_comp_subset φ S T hT_sub, infinite_setOf_nonempty_inter φ S T hT_infSet⟩
